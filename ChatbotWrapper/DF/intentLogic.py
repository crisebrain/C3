...
def analizeReq(req: dict, specjson):
    value = None
    contador = 1
    ...
    dicResponseIntents = specjson["dicResponseIntents"]
  ...
  #
    ...
    # Caso donde primero hicieron pregunta implícita, y luego me especifican de qué hablan.
        ...
        print("IMPLICITY CASE ATTENDED " + str(dicResponseIntents[intentName.split("-")[0]]))
        respIntent = dicResponseIntents[intentName.split("-")[0]][value]
    ...
    # Pregunta explícita. Dan la entity en la misma pregunta.
        ...
        try:
            print("EXPLICITY CASE ATTENDED " + str(dicResponseIntents[intentName][value]))
            respIntent = dicResponseIntents[intentName][value]
            contextsHistory.append(dicEntityToContext[value])
            print(str(contextsHistory))
            #return createResp(respIntent, value)
            _definedContext = True
        except:
            print("EXPLICITY CASE ATTENDED " + str(dicResponseIntents[intentName]))
            respIntent = dicResponseIntents[intentName]
            #contextsHistory.append(dicEntityToContext[value])
            #print(str(contextsHistory))
